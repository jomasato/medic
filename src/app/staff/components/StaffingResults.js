        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm">薬剤師稼働率</span>
                <span className={`font-bold ${getUtilizationColor(staffing.pharmacistUtilization)}`}>
                  {staffing.pharmacistUtilization}%
                </span>
              </div>
              {staffing.clerks > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-sm">事務員稼働率</span>
                  <span className={`font-bold ${getUtilizationColor(staffing.clerkUtilization)}`}>
          ...
          {/* Optimal Staffing */}
          {(staffing.pharmacistUtilization > 75 || (staffing.clerks > 0 && staffing.clerkUtilization > 80)) && (
                ...
                <AlertDescription>
                  <div className="ml-2">
                    ...
                    <ul className="list-disc list-inside mt-1">
                      {staffing.pharmacistUtilization > 75 && (
                        <li>
                          薬剤師：{staffing.optimalPharmacists}名（現在：{staffing.pharmacists}名）
                          <span className="text-sm text-gray-600 ml-2">
                            ※目標稼働率75%を確保するための推奨人数
                          </span>
                        </li>
                      )}
                      {staffing.clerks > 0 && staffing.clerkUtilization > 80 && (
                        <li>
                          事務員：{staffing.optimalClerks}名（現在：{staffing.clerks}名）
                          <span className="text-sm text-gray-600 ml-2">
                            ※目標稼働率80%を確保するための推奨人数
                          </span>
                        </li>
                      )}
                    </ul>
